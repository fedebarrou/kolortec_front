  ...
  return (
    <section className="relative flex min-h-[calc(100svh-72px)] md:min-h-[calc(100svh-80px)] items-end overflow-hidden kt-section-reveal" style={{ '--reveal-delay': '10ms' }}>
      ...
      <div className="relative z-[2] px-6 pb-[100px] lg:px-40 kt-hero-content-enter">
        <div className="mb-3 inline-block bg-primary px-3.5 py-1.5 text-sm font-black text-[#090909]">{hero.badge}</div>
        <h1 className="title-font m-0 mb-4 text-[clamp(3.6rem,10vw,8.4rem)] leading-[0.95] tracking-[0]">
          {heroTitleBase}
          <span className="text-primary">.</span>
        </h1>
        <p className="m-0 max-w-[680px] text-[clamp(1rem,2.5vw,1.2rem)] text-[#d4d4d4]">{heroSubtitle}</p>
        <div className="mt-6 flex flex-wrap gap-3">
          <button className="rounded-[8px] bg-primary px-6 py-3.5 text-sm font-extrabold uppercase tracking-[0.12em] text-[#090909] transition hover:-translate-y-0.5" type="button">
            {hero.primaryCta}
          </button>
          <button className="rounded-[8px] border-2 border-white bg-transparent px-6 py-3.5 text-sm font-extrabold uppercase tracking-[0.12em] text-white transition hover:bg-white hover:text-[#090909]" type="button">
            {hero.secondaryCta}
          </button>
        </div>
      </div>
